...
(*19 April 2021: Just update version to 5.0; Improve parsing info files 	(BUT cannot read fields on MULTIPLE lines )*)
(**)
...
(* ::Input::Initialization:: *)
(* Modified for v5; improved parsing of text strings with ' and "  *)
(* This reads info files *)
readInfo[fileName_,verbose_:True]:=
Module[{debug=False,stream,tmp,tmp2,tmp3={},tmp4,tmp5,rules,infotable0,alphainfo,flavorlist},
stream=OpenRead[fileName];
SetStreamPosition[stream,0];

Do[
tmp= Read[stream,Record];
If[tmp==EndOfFile,Break[]]; (* Read individual lines.  *)
tmp2=StringSplit[tmp,":",2]; (* ONLY SPLIT AT FIRST : *)
If[debug,Print["=== i= ",i,"  ",tmp2]];
...
alphainfo={"n/a","n/a"},
alphainfo={ "AlphaS_Qs"/.rules,"AlphaS_Vals"/.rules};
];(*End If...*)
...
(* ::Input::Initialization:: *)
(* This is version 4.0 and below. 
We've improved so not to get confused about " and ' in text strings *)

(* This reads info files *)
readInfoOLD[fileName_,verbose_:True]:=
